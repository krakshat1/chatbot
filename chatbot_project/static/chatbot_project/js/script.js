...
"use strict";
    ...
    function updateChatText() {
      let html = "";
      let botHtmlTag = '<div class = "bot-message-block floatup-msg"><img class = "bot-profile" src="{% static "chatbot_project/images/download.png"%}"><p class= bot_message>';
    ...
    function showBotFun() {
      ...
      if (messages.length === 0) {
        let msg2 = { name: 'Bot', message: '<form id="myForm" value="query"> <div id="chatbot-chat"><div><button class="chat-btn  mt-1 " value = "click here" name ="query" id = "query" onclick = "checkText()">Click Here 👀</button> <button class="chat-btn  mt-1">Just browsing 👀</button><button class="chat-btn mt-1">I have questions 😊</button><button class="chat-btn  mt-1">Using ChatBot 👉 </button></form></div>'};
        messages.push(msg2);
        updateChatText();
        scrollToBottom();
      }
